...
static GLNVGtexture* glnvg__allocTexture(GLNVGcontext* gl)
{
	...
	if (tex == NULL) {
		if (gl->ntextures+1 > gl->ctextures) {
			...
			textures = (GLNVGtexture*)SDL_realloc(gl->textures, sizeof(GLNVGtexture)*ctextures);
...
static GLNVGcall* glnvg__allocCall(GLNVGcontext* gl)
{
	...
	if (gl->ncalls+1 > gl->ccalls) {
		...
		calls = (GLNVGcall*)SDL_realloc(gl->calls, sizeof(GLNVGcall) * ccalls);
...
static int glnvg__allocPaths(GLNVGcontext* gl, int n)
{
	...
	if (gl->npaths+n > gl->cpaths) {
		...
		paths = (GLNVGpath*)SDL_realloc(gl->paths, sizeof(GLNVGpath) * cpaths);
...
static int glnvg__allocVerts(GLNVGcontext* gl, int n)
{
	...
	if (gl->nverts+n > gl->cverts) {
		...
		verts = (NVGvertex*)SDL_realloc(gl->verts, sizeof(NVGvertex) * cverts);
...
static int glnvg__allocFragUniforms(GLNVGcontext* gl, int n)
{
	...
	if (gl->nuniforms+n > gl->cuniforms) {
		...
		uniforms = (unsigned char*)SDL_realloc(gl->uniforms, structSize * cuniforms);
...
static void glnvg__renderDelete(void* uptr)
{
	...
	SDL_free(gl->textures);

	SDL_free(gl->paths);
	SDL_free(gl->verts);
	SDL_free(gl->uniforms);
	SDL_free(gl->calls);
...
#endif
{
	NVGparams params;
	NVGcontext* ctx = NULL;
	GLNVGcontext* gl = (GLNVGcontext*)SDL_malloc(sizeof(GLNVGcontext));
