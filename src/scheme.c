...
static pointer port_from_tcp(scheme *sc, TCPsocket skt);
...
static pointer opexe_4(scheme *sc, enum scheme_opcodes op) {
	...
	case OP_OPEN_NETSOCKET: {
		 unsigned short port = ivalue(car(sc->args));
		 pointer p;
...
static int is_any(pointer p) { (void) p; return 1;}
